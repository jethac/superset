...
class KustoKqlEngineSpec(BaseEngineSpec):  # pylint: disable=abstract-method
    ...
    metadata = {
        "description": (
            "Azure Data Explorer (Kusto) is a fast, fully managed data analytics "
            "service from Microsoft Azure. This spec queries it in its native "
            "Kusto Query Language rather than SQL."
        ),
        "logo": "kusto.png",
        "homepage_url": "https://azure.microsoft.com/en-us/products/data-explorer/",
        "categories": [
        ...
        "notes": (
            "The cluster is addressed over HTTPS, so no port is configured. "
            "`sqlalchemy-kusto` serves both dialects: the `kustosql` engine "
            "exposes the same clusters through the SQL endpoint."
        ),
    }
